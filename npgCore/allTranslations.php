<?php
/* This file contains language strings extracted from getAllTranslations() function calls.
 * it is used by Poedit to capture the strings for translation.
 */

/* themes/effervescence+/themeoptions.php */
gettext('All');
gettext('Articles Rule');
gettext('Gallery');
//gettext('All');
gettext('Albums');
gettext('Gallery Rule');
gettext('Pages');
//gettext('All');
gettext('Pages Rule');
gettext('Archive');
//gettext('All');
gettext('Archive Rule');
gettext('RSS');
//gettext('Gallery');
gettext('Both');

/* themes/garland/themeoptions.php */
//gettext('All');
//gettext('Articles Rule');
//gettext('Gallery');
//gettext('All');
//gettext('Gallery Rule');
//gettext('Pages');
//gettext('All');
//gettext('Pages Rule');
//gettext('RSS');
//gettext('Gallery');
//gettext('Both');

/* themes/zenpage/themeoptions.php */
//gettext('All');
//gettext('Articles Rule');
//gettext('All');
gettext('Album list');
//gettext('Gallery Rule');
//gettext('Pages');
//gettext('All');

/* core//extensions/contact_form.php */
gettext('<p>Fields with <strong>*</strong> are required. HTML or any other code is not allowed.</p>');
gettext('<p>Please confirm that you really want to send this email. Thanks.</p>');
gettext('<p>Thanks for your message.</p>');
gettext('Send another message.');
gettext('<p>A copy of your e-mail will automatically be sent to the address you provided for your own records.</p>');

/* core//extensions/cookieconsent.php */
gettext('Agree');
gettext('More info');
gettext('This website uses cookies. By continuing to browse the site, you agree to our use of cookies.');

/* core//extensions/favoritesHandler.php */
gettext('My favorites');
//gettext('My favorites');
gettext('The albums and images selected as favorites.');
gettext('Add favorite');
gettext('Remove favorite');

/* core//extensions/galleryArticles.php */
gettext('New album: %1$s');
gettext('New image: [%2$s]%1$s');

/* core//extensions/paged_thumbs_nav.php */
gettext('« prev thumbs');
gettext('next thumbs »');

/* core//extensions/register_user.php */
gettext('Click here to register for this site.');
gettext('Register');
gettext('You have received this email because you registered with the user id %3$s on this site.' . "<br />" . 'To complete your registration visit %1$s.');
gettext('Your registration information has been accepted. An email has been sent to you to verify your email address.');

/* core//extensions/zenpage.php */
gettext('Read more');

/* core//setup/setup-option-defaults.php */
gettext("What is your father’s middle name?");
gettext("What street did your Grandmother live on?");
gettext("Who was your favorite singer?");
gettext("When did you first get a computer?");
gettext("How much wood could a woodchuck chuck if a woodchuck could chuck wood?");
gettext("What is the date of the Ides of March?");
gettext('Check to acknowledge the site <a href="%s">usage policy</a>.');
gettext('Users with full privileges');
gettext('Users allowed only to view and comment');
gettext('Banned users');
gettext('Managers of one or more albums');
gettext('Default user settings');
gettext('Newly registered and verified users');
gettext('News');
?>