...
function printAdminHeader($tab, $subtab = NULL) {
	...
	<html<?php i18n::htmlLanguageCode(); ?>>
		<head>
			...
			scriptLoader(CORE_SERVERPATH . 'js/admin.js', 'async');
			scriptLoader(CORE_SERVERPATH . 'js/jquery.scrollTo.min.js', 'async');

			if (extensionEnabled('touchPunch')) {
				scriptLoader(CORE_SERVERPATH . 'js/jquery.ui.touch-punch.min.js', 'async');
			}
			if ($multi) {
	...
	?>
		...
		function printSortableHead() {
			scriptLoader(CORE_SERVERPATH . 'js/jquery.mjs.nestedSortable.js', 'async')
			?>
			<!--Nested Sortables-->
			<script>
				//<!-- <![CDATA[
	...
	function printAdminFooter($addl = '') {
		...
		<div id="footer">
			...
			<span id="footer_right">
				...
				| <a href="https://<?php echo GITHUB; ?>/discussions" title="<?php echo gettext('netPhotoGraphics Discussions'); ?>"><?php echo gettext('Discussions'); ?></a>
		...
		if (TEST_RELEASE) {
			...
			if (count($_Script_processing_timer) > 1) {
				printf("<!-- " . gettext('Script processing total:%.6f seconds') . " -->\n", $last - $first);
			}
		}
	}
	...
	/**
	 * generates a list of albums that are authorized by the rights.
	 * Note: dynamic albums do not grant upload rights so are not returned if
	 * UPLOAD rights are requested.
	 *
	 * @global obj $_gallery
		...
		if (is_array($albums)) {
			foreach ($albums as $folder) {
				...
				if ($album->isDynamic()) {
					...
					$list[$album->getFileName()] = $album->getTitle();
					$list = array_merge($list, genAlbumList($album, $rights)); /* generate for subalbums */
				}
			}
		}
		return $list;
	 ...
	 */
	function generateUnorderedListFromArray($currentValue, $list, $prefix, $alterrights, $sort, $localize, $class = false, $extra = NULL, $postArray = 0) {
		if (is_null($extra))
			$extra = array();
		if (!empty($class))
			$class = ' class="' . $class . '" ';
	...
	function sortTagList($tagsort = 'alpha') {
		...
		if (is_null($_admin_ordered_taglist) || $tagsort != $_admin_ordered_taglist_order) {
			...
			switch ($tagsort) {
				case 'language':
					$order = '`language` DESC,`name` COLLATE ' . UTF8MB4_AS_DISPLAY_COLLATION;
					break;
				case 'recent':
					$order = '`id` DESC';
					break;
				case 'mostused':
					...
					break;
				case 'private':
					$order = '`private` DESC, `name` COLLATE ' . UTF8MB4_AS_DISPLAY_COLLATION;
					break;
				default:
					$order = '`name` COLLATE ' . UTF8MB4_AS_DISPLAY_COLLATION;
					break;
			}
	...
	function printAlbumButtons($album) {
		if (!$album->isDynamic() && npg_loggedin(UPLOAD_RIGHTS)) {
			npgButton('button', ARROW_UP_GREEN . ' ' . gettext('Upload to album'), array('buttonLink' => getAdminLink('admin-tabs/upload.php') . '?album=' . $album->name, 'buttonTitle' => gettext("Upload"), 'buttonClass' => 'fixedwidth'));
			?>
			<br /><br />
			<?php
		}
			...
			<div class="page-list_iconwrapper">
				...
				<div class="page-list_icon">
					...
					if (!$enableEdit || $supress) {
						...
						?>
						<a class="delete" href="javascript:confirmDeleteAlbum('?page=edit&amp;action=deletealbum&amp;album=<?php echo urlencode(pathurlencode($album->name)); ?>&amp;return=<?php echo urlencode(pathurlencode($owner)); ?>&amp;XSRFToken=<?php echo getXSRFToken('delete') ?>');" title="<?php echo sprintf(gettext("Delete the album %s"), js_encode($album->name)); ?>">
							<?php echo WASTEBASKET; ?>
						</a>
						<?php
					}
	...
	function printImagePagination($album, $image, $singleimage, $allimagecount, $totalimages, $pagenum, $totalpages, $filter) {
		if ($singleimage) {
			...
			if ($allimagecount != $totalimages) { // need pagination links
				?>
				<span align="center">					<?php adminPageNav($pagenum, $totalpages, 'admin-tabs/edit.php', '?page=edit&amp;album=' . pathurlencode($album->name), '&amp;tab=imageinfo&amp;filter=' . $filter); ?>				</span>
				<?php
			}
		}
	}
...
// If the target theme already exists, nothing to do.
	...
	function currentRelativeURL() {
		$source = str_replace('\\', '/', $_SERVER['SCRIPT_NAME']);
		$source = str_replace(WEBPATH . '/' . CORE_FOLDER . '/', '', $source);
...
function printManagedObjects($type, $objlist, $alterrights, $userobj, $prefix_id, $kind, $flag) {
		...
		case 'albums':
			if ($rights & (MANAGE_ALL_ALBUM_RIGHTS | ADMIN_RIGHTS)) {
				...
				if (!($rights & VIEW_UNPUBLISHED_RIGHTS))
					$legend .= EXCLAMATION_RED . ' ' . gettext('view unpublished');
					...
					if ($item['type'] == 'albums') {
						...
						if (($rights & UPLOAD_RIGHTS)) {
							if (hasDynamicAlbumSuffix($item['data']) && !is_dir(ALBUM_FOLDER_SERVERPATH . $item['data'])) {
								$extra[$item['data']][] = array('name' => 'upload', 'value' => MANAGED_OBJECT_RIGHTS_UPLOAD, 'display' => ARROW_UP_GRAY, 'checked' => 0, 'disable' => true);
							} else {
								$extra[$item['data']][] = array('name' => 'upload', 'value' => MANAGED_OBJECT_RIGHTS_UPLOAD, 'display' => ARROW_UP_GREEN, 'checked' => $item['edit'] & MANAGED_OBJECT_RIGHTS_UPLOAD);
							}
						}
						if (!($rights & VIEW_UNPUBLISHED_RIGHTS)) {
				...
				foreach ($rest as $unmanaged) {
					...
					if (($rights & UPLOAD_RIGHTS)) {
						if (hasDynamicAlbumSuffix($unmanaged) && !is_dir(ALBUM_FOLDER_SERVERPATH . $unmanaged)) {
							$extra2[$unmanaged][] = array('name' => 'upload', 'value' => MANAGED_OBJECT_RIGHTS_UPLOAD, 'display' => ARROW_UP_GRAY, 'checked' => 0, 'disable' => true);
						} else {
							$extra2[$unmanaged][] = array('name' => 'upload', 'value' => MANAGED_OBJECT_RIGHTS_UPLOAD, 'display' => ARROW_UP_GREEN, 'checked' => 1);
						}
					}
					if (!($rights & VIEW_UNPUBLISHED_RIGHTS)) {
	...
	if (empty($alterrights) && (!empty($rest) || !empty($cv))) {
		?>
 ...
 * normally used to exclude these scripts form various option selectors.
 *
 ...
 * @param int $isLatitude
 * @return string
 */
function parseDMS($geoString, $isLatitude) {
...
function fetchGithubLatest($owner, $repro, $option, $required = true) {
	...
	try {
		...
		$fullRepoResponse = $api->get('/repos/:owner/:repo/releases/latest', array('owner' => $owner, 'repo' => $repro));
