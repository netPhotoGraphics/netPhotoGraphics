...
function printUnpublishedDropdown() {
	...
	<form name="AutoListBox3" id="unpublisheddropdown" style="float:left; margin:5px;"	action="#">
		<select name="ListBoxURL" size="1"	onchange="npg_gotoLink(this.form)">
			...
			echo "<option $all value='" . getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath($option) . "'>" . gettext("All articles") . "</option>\n";
			echo "<option $published value='" . getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath(array_merge(array('published' => 'yes'), $option)) . "'>" . gettext("Published") . "</option>\n";
			echo "<option $unpublished value='" . getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath(array_merge(array('published' => 'no'), $option)) . "'>" . gettext("Un-published") . "</option>\n";
			echo "<option $sticky value='" . getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath(array_merge(array('published' => 'sticky'), $option)) . "'>" . gettext("Sticky") . "</option>\n";
			?>
		</select>
	...
	<form name="AutoListBox4" id="sortorderdropdown" style="float:left; margin:5px;"	action="#">
		<select name="ListBoxURL" size="1"	onchange="npg_gotoLink(this.form)">
			...
			foreach ($selections as $sortorder => $text) {
				?>
				<option<?php if ($sortorder == $selected) echo ' selected="selected"'; ?> value="<?php echo getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath(array_merge(array('sortorder' => $sortorder), $option)); ?>"><?php echo $text; ?></option>
				<?php
			}
			?>
		</select>
...
function printCategoryDropdown() {
	...
	if (count($result) > 0) {
		...
		<form name ="AutoListBox2" id="categorydropdown" style="float:left; margin:5px;" action="#" >
			<select name="ListBoxURL" size="1" onchange="npg_gotoLink(this.form)">
				<?php
				echo "<option $selected value='" . getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath($option) . "'>" . gettext("All categories") . "</option>\n";
				if ($category == '`') {
					$selected = "selected='selected'";
				} else {
					$selected = "";
				}
				echo "<option $selected value='" . getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath(array_merge(array(
	...
	<form name="AutoListBox5" id="articlesperpagedropdown" method="POST" style="float:left; margin:5px;"	action="#">
		<select name="ListBoxURL" size="1"	onchange="npg_gotoLink(this.form)">
			...
			foreach ($list as $count) {
				?>
				<option <?php if ($articles_page == $count) echo 'selected="selected"'; ?> value="<?php echo getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath(array_merge(array('articles_page' => $count, 'subpage' => (int) ($subpage * $articles_page / $count)), $option)); ?>"><?php printf(gettext('%u per page'), $count); ?></option>
				<?php
			}
			?>
			<option <?php if ($articles_page == 0) echo 'selected="selected"'; ?> value="<?php echo getAdminLink(PLUGIN_FOLDER . '/zenpage/news.php') . getNewsAdminOptionPath(array_merge(array('articles_page' => 'all'), $option)); ?>"><?php echo gettext("All"); ?></option>
		</select>
...
function zenpageJSCSS() {
	scriptLoader(PLUGIN_SERVERPATH . 'zenpage/zenpage.css');
}
