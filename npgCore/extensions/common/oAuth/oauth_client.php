...
class oauth_client_class {
	...
	Function ProcessToken1($oauth, &$access_token) {
		...
		if (IsSet($response['oauth_expires_in']) && $response['oauth_expires_in'] == 0) {
			...
			$this->access_token_expiry = gmtime('Y-m-d H:i:s', time() + $expires);
			if ($this->debug)
	...
	Function RetrieveToken(&$valid) {
		...
		if (IsSet($access_token['value'])) {
			$this->access_token_expiry = '';
			$expired = (IsSet($access_token['expiry']) && strcmp($this->access_token_expiry = $access_token['expiry'], gmtime('Y-m-d H:i:s')) < 0);
			if ($expired) {
				if ($this->debug)
	...
	Function CallAPI($url, $method, $parameters, $options, &$response) {
		...
		switch (intval($this->oauth_version)) {
			case 1:
				if (strlen($this->access_token_expiry) && strcmp($this->access_token_expiry, gmtime('Y-m-d H:i:s')) <= 0) {
		...
		switch (intval($this->oauth_version)) {
			case 1:
				...
				if (IsSet($access_token['authorized']) && IsSet($access_token['value'])) {
					$expired = (IsSet($access_token['expiry']) && strcmp($access_token['expiry'], gmtime('Y-m-d H:i:s')) <= 0);
