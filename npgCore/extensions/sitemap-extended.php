...
class sitemap {
	...
	static function getGoogleImageVideoExtras($albumobj, $imageobj, $locale) {
		...
		if ($imageobj->isVideo() && in_array($ext, array('.mpg', '.mpeg', '.mp4', '.m4v', '.mov', '.wmv', '.asf', '.avi', '.ra', '.ram', '.flv', '.swf'))) { // google says it can index these so we list them even if unsupported
