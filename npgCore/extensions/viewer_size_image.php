...
$plugin_disable = extensionEnabled('panorama') ? gettext('Viewer sized image is not compatible with the <a href="#panorama"><code>panorama</code></a> plugin.') : '';
		...
		if ($selector) {
			?>
				function switchselection() {
					...
					document.cookie = 'viewer_size_image_saved=' + items[0] + '; path=<?php echo $cookiepath ?>';
				}
			<?php
		} else { //	radio buttons
			?>
				function switchimage(obj) {
					...
					document.cookie = 'viewer_size_image_saved=' + $(obj).attr('value') + '; path=<?php echo $cookiepath ?>';
				}
			<?php
		}
		?>
