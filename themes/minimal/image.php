...
<div id="content">
	<div id="main"<?php if ($zpmin_switch) echo ' class="switch"'; ?>>
		...
		<div id="image-wrap">
			<div id="full-image">
				<?php if (($zpmin_finallink) == 'colorbox') { ?><a class="thickbox" href="<?php echo html_encode(getUnprotectedImageURL()); ?>" title="<?php echo html_encode(getBareImageTitle()); ?>"><?php printCustomSizedImage(getAnnotatedImageTitle(), array('size' => 550)); ?></a><?php } ?>
				<?php
				if (($zpmin_finallink) == 'nolink') {
					printCustomSizedImage(getAnnotatedImageTitle(), array('size' => 550));
				}
				?>
				<?php if (($zpmin_finallink) == 'standard') { ?><a href="<?php echo html_encode(getFullImageURL()); ?>" title="<?php echo html_encode(getBareImageTitle()); ?>"><?php printCustomSizedImage(getAnnotatedImageTitle(), array('size' => 550)); ?></a><?php } ?>
				<?php if (($zpmin_finallink) == 'standard-new') { ?><a target="_blank" href="<?php echo html_encode(getFullImageURL()); ?>" title="<?php echo html_encode(getBareImageTitle()); ?>"><?php printCustomSizedImage(getAnnotatedImageTitle(), array('size' => 550)); ?></a><?php } ?>
			</div>
		</div>
		<?php
	...
	<div id="sidebar"<?php if ($zpmin_switch) echo ' class="switch"'; ?>>
		...
		<div class="sidebar-divide">
			<div id="full-image-details">
				...
				<?php } ?>
				<?php if (function_exists('printSlideShowLink' && $_current_image->isPhoto())) { ?><div class="sidebar-section"><div class="slideshow-link"><?php printSlideShowLink(gettext('View Slideshow')); ?></div></div><?php } ?>
			</div>
		</div>
