	...
	switch ($_gallery_page) {
		...
		case 'image.php':
			...
			$comments_allowed = $_current_image->getCommentsAllowed();
			break;
		case 'pages.php':
			$comments_open = getOption('comment_form_pages');
			$comments_allowed = $_CMS_current_page->getCommentsAllowed();
			break;
		case 'news.php':
			$comments_open = getOption('comment_form_articles');
			$comments_allowed = $_CMS_current_article->getCommentsAllowed();
			break;
		default:
			return;
			break;
	...
	<?php if (($comments_open) && ($comments_allowed || (getCommentCount() > 0 ))){ ?>
		<a class="fadetoggler"><img src="<?php echo $_themeroot; ?>/images/icon-comment.png" alt="icon-comment" id="icon-comment" />
		<?php
		$num = getCommentCount();
		if ($num == 0) {
